...
export let cardsManager = {
  loadCards: async function (boardId) {
    ...
    for (let card of cards) {
      ...
      domManager.addChild(
        `.board-columns[data-board-id="${boardId}"] .board-column[data-column-id="${card.status_id}"] .board-column-content`,
        content
      );
      ...
      dragHandler.initDraggable(
        document.querySelector(`.card[data-card-id="${card.id}"]`)
      );
    }
  },
  addCardHandler: async function (clickEvent) {
    ...
    if (
      ...
      domManager.addChild(
        `.board-columns[data-board-id="${boardId}"] .board-column[data-column-id="${newCard.status_id}"] .board-column-content`,
        content
      );
      domManager.addEventListener(
